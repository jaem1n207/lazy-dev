...
const PostCard = ({ post }: PostCardProps) => {
  ...
  return (
    <Link
      ...
      className="focus-primary row-span-5 row-end-auto grid grid-rows-article gap-2 rounded-lg p-1"
    >
      <GatsbyImage
      ...
      <div className="flex flex-wrap gap-8pxr">
        {tags?.map((tag) => (
          <span key={tag} className="font-semibold text-primary tablet:text-14pxr">
            {tag}
          </span>
        ))}
      </div>
